...
def optimize(data: ModelData, method: str, allocate_all_resources = True, verbose=False):
    ...
    if data.n >= 2000:
        for u in data.users.values():
            ...
            print("g_req =",u.g_req)
            print("f_req =",u.f_req)
            print("hist_r =",u.hist_r)
            print("hist_d =",u.hist_d)
            print("hist_rcv =",u.hist_rcv)
            print("hist_sent =",u.hist_sent)
            print("hist_buff =",u.hist_buff)
            print("acc =",u.acc)
            print("\n")
    ...
    # SET: U
    m.U = pyo.Set(initialize = data.users.keys())

    # SET: U_rlp
    m.U_rlp = pyo.Set(initialize = [u.id for u in data.users.values() if u.s == "embb" or u.s == "urllc"])

    # SET: U_fg
    m.U_fg = pyo.Set(initialize = [u.id for u in data.users.values() if u.s == "be"])

    # SETS: U_{s}
    U_s = dict()
    U_s["embb"] = [u.id for u in data.users.values() if u.s == "embb"]
    U_s["urllc"] = [u.id for u in data.users.values() if u.s == "urllc"]
    U_s["be"] = [u.id for u in data.users.values() if u.s == "be"]
    m.U_embb = pyo.Set(initialize = U_s["embb"])
    m.U_urllc = pyo.Set(initialize = U_s["urllc"])
    m.U_be = pyo.Set(initialize = U_s["be"])
